...
- 与 `ClassLinker` 内部的 `bootPandaFiles_` 对齐（见 [FileNotes/runtime_class_linker.cpp](runtime_class_linker.cpp.md) 的 AddPandaFile 段）。
...
- 抽象与数据成员：[FileNotes/runtime_include_class_linker_extension.h](runtime_include_class_linker_extension.h.md)
- `ClassLinker` 主管线/FreeClassData：[FileNotes/runtime_class_linker.cpp](runtime_class_linker.cpp.md)
- core 语言实现（roots 与 CreateClass）：`runtime/core/core_class_linker_extension.cpp`（将补齐逐行）
- ETS 语言实现：[FileNotes/plugins_ets_runtime_ets_class_linker_extension.cpp](plugins_ets_runtime_ets_class_linker_extension.cpp.md)
